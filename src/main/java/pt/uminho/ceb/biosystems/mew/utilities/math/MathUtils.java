...
public class MathUtils
{
  ...
  public static double normal(double u, double s) 
	...
	public static Pair<Number, Number> minMax(Collection<Number> numbers) {
	...
	public static <T extends Number> Pair<T, T> minMaxT(Collection<T> numbers) {
	...
	// sums the absolute values of an int[]
	public static int sumIntArray(int[] double_array){
		int sum = 0;
		
		for(int d: double_array)
			sum+=Math.abs(d);
	...
	public static int[] firstCombination(int n){
		if(n<=0)
			return null;
		else{
			int[] ret = new int[n];
	...
	public static boolean[] nextCombinationV2(boolean[] s, boolean increase) {
		boolean [] nextS = new boolean[s.length];
		
		int pos = 0;
		
		while(pos < nextS.length && s[pos] != increase) {
	...
	public static double truncate(double value, int nDecimals){
		BigDecimal r = new BigDecimal(value);
		return r.setScale(nDecimals, BigDecimal.ROUND_HALF_DOWN).doubleValue();
	}
	...
	public static long gcd(long [] array) {
	  ...
	  // iterative version of factorial (memory efficient), using BigInteger representation
	  public static BigInteger factorial(int n_i){
	  ...
	  //gives maximum number of combinations from n elements r to r (no repeats, no order)
	  public static BigInteger numPerms(int n, int r){
//		long facN = factorial(n);
	  ...
	  public static void main(String... args){
		  ...
		  while(current!=null){
			  ...
			  int[] next = nextCombination(current, n);
		  
			  System.out.println(Arrays.toString(next)+" ("+k+")");
			  current = next;
			  k++;
		  }
	...
	public static BigInteger pow(long base, long power) {
